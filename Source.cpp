...
void levenshtein(const std::string& p, const std::string& t, bool withTranspositions = false)
{
...
#define _table(i,j) table[(i)*n + (j)]
    ...
    while (i && j)
    {
        ...
        if (withTranspositions && i > 1 && j > 1 && _table(i, j) == _table(i - 2, j - 2) + 1 && p[i - 1] == t[i - 2] && p[i - 2] == t[j - 1])
        {
...
size_t fullDynProg(const std::string& p, const std::string& t, size_t k)
{
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        if (current[m] <= k) ++matches; // matching substring found at position i+1
...
size_t cutOffDynProg(const std::string& p, const std::string& t, size_t k)
{
    const size_t m = p.length(), n = t.length();
    if (k >= m) return 0;
    std::vector<size_t> previous(m + 1);
    ...
    for (size_t i = 1; i <= n; i++)
    {
        ...
        for (size_t j = 1; j < currSize; j++)
        {
            ...
            if (j <= lp) current[j] = min(current[j], previous[j] + 1);
            if (current[j] <= k) lc = j;
        }
...
size_t diagTransitions(const std::string& p, const std::string& t, size_t k)
{
    ...
    for (int_t j = 2; j < table_w; j++)
    {
        ...
        if (current[k + 1] == m + j - k - 2) ++matches; // matching substring found at position current[k + 1]
...
size_t bitParalelized(const std::string& p, const std::string& t, size_t k)
{
    ...
    for (size_t i = 1; i <= n; i++)
    {
        ...
        pv = (mh << 1) | (~(xv | ph));
        mv = ph & xv;
        if (score <= k) ++matches; // matching substring found at position i
    }
    return matches;
}
...
int main()
{
    ...
    levenshtein("bananas", "pyjamas");
    // When allowing transpositions affects the result:
    levenshtein("andi", "nadq");
    levenshtein("andi", "nadq", true);
